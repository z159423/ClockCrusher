        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Set Material\n인게임에서 파티클 안나오면 누르기"))
            {
                ...
                foreach (var mat in current.materials)
                {
                    ...
                    if (lastWord != "UI")
                    {
                        string oriPath = AssetDatabase.GetAssetPath(mat);
                        ...
                        string matPath = oriPath.Substring(6, lastIndex - 6);
                        //해당 경로에 이름_UI가 있는지 체크
                        var uiMatFile = System.IO.Directory.GetFiles(Application.dataPath + matPath, mat.name + "_UI.mat", System.IO.SearchOption.AllDirectories);

                        //있으면 그걸로 바꾸기
                        if (uiMatFile.Length > 0)
                        {
                            int assetsIndex = uiMatFile[0].IndexOf("Assets/");
                            uiMatFile[0] = uiMatFile[0].Substring(assetsIndex);

                            Debug.Log($"Change Material to {uiMatFile[0]}");

                            current.particles[particleInex++].GetComponent<ParticleSystemRenderer>().material = UnityEditor.AssetDatabase.LoadAssetAtPath<Material>(uiMatFile[0]);
                        }
                        //없으면
                        else
                        {
                            //새로운 mat 만들어주기
                            Texture oriTexture = mat.GetTexture("_MainTex");

                            Material material = new Material(Shader.Find("UI/Unlit/Transparent"));
                            material.mainTexture = oriTexture;

                            string newAssetName = "Assets" + matPath + "/" + mat.name + "_UI.mat";

                            AssetDatabase.CreateAsset(material, newAssetName);
                            AssetDatabase.SaveAssets();

                            current.particles[particleInex++].GetComponent<ParticleSystemRenderer>().material = material;

                            Debug.Log($"New Material at {newAssetName}");
